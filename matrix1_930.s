;------------------------------------------------------------------
;----------MOLTIPLICAZIONE TRA MATRICI (RIGA PER COLONNA)----------
;------------------------------------------------------------------
;------------------------------------------------------------------
;--------------------SCRITTO DA EDOARDO SPADONI--------------------
;------------------------------------------------------------------
...
loop:		
		; Lettura elemento in posizione matX[i][k]
			MULT    r5, r2, r1			; i * max
			ADD     r6, r5, r4			; i * max + k
			SLL     r7, r6, 3			; Posizione elemento matX
			LD      f2, matX(r7)		; Leggo elemento di matX
				
		; Lettura elemento in posizione matY[k][j]
			MULT    r5, r4, r1			; k * max
			ADD     r6, r5, r3			; k * max + j
			SLL     r7, r6, 3			; Posizione elemento di matY
			LD      f4, matY(r7)		; Leggo elemento di matY
		...
		; Incremento k e controllo il suo valore
			ADD     r4, r4, 1			; k = k + 1
			SEQ     r8, r4, r1			; Controllo se k = max
			BEQZ    r8, loop			; Se k != max vai a 'loop' altrimenti continua il codice
		
		; Salvo il valore del registro in matZ
			MULT    r9, r2, r1			; i * max
			ADD     r10, r9, r3			; i * max + j
			SLL     r11, r10, 3			; Posizione elemento matZ
			SD      matZ(r11), f0		; Salvo il valore di f0 in matZ[i][j]
	
		; Incremento j e controllo il suo valore
			ADD     r3, r3, 1			; j = j +1
			SEQ     r12, r3, r1			; Controllo se j = max
			BEQZ    r12, zero_k			; Se j != max vai a 'zero_k' altrimenti continua il codice
	
		; Incremento i e controllo il suo valore
			ADD     r2, r2, 1			; i = i + 1
			SEQ     r13, r2, r1			; Controllo se i = max
			BEQZ    r13, zero_j			; Se i != max vai a 'zero_j' altrimenti continua il codice
